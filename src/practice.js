...
const practiceLessons=[
 ['先走到地上的圆圈','按 W 向前走。把自己移动到前方的白色起步圈里；不用跳，也没有时间限制。','WASD 移动','这里通过移动、观察和改变物体解谜，没有战斗。',0,13,.15],
 ['用准星选中方块，再拿起来','移动鼠标，让屏幕中心的小圆点落在三点方块上，再按 E。若鼠标不转动，按住右键拖动或用方向键。','准星对准方块 → E','能互动时，准星会变大，并显示“E 拿起”。',0,9,.5],
 ['让方块真的变大','拿着方块，抬到平视前方。按 + 或向上滚动滚轮，把尺寸从 0.60 m 增大到至少 1.40 m。','+ / 滚轮向上 推远','拿住时，方块在画面中看起来差不多大；把它推到更远处，实际尺寸就会变大。',null,null,null],
 ['把变大的方块放在圆环上','保持方块至少 1.40 m。转向右侧右侧实线圆环，低头调整落点，E 放下。走近或侧移可以调整位置。','E 放下','放下后，尺寸会保留下来。只有落在圆环中心、大小合适并松手，圆环才会亮。',3,4,.2],
 ...
 ['让照片里的方块落进现实','转向左侧空地。滚轮调整预览的远近；显示“可显影”后按 F。若仍是灰暗，移开自己或避开原方块。','可显影预览 → F','F 把照片里的东西放进世界，这叫“显影”。成功后，原件和副本会同时存在。',-3,4,.2],
 ...
 ['把桥的两端搭在岸上','走到断崖前的白色脚印处，面朝对岸。C 举桥照片，滚轮调整到显示“可显影”，F 放下；照片会变成一座桥。','C 预览 → 显示“可显影”时 F','桥的起点和终点都要落在平台上。灰暗表示两端还没接好，此时不能放置。',0,-5.5,.15],
...
function buildPracticePrevious(){
  setupLight();scene.background.set('#626262');floor(24,28,0,6,0,'@tile');floor(16,10,0,-23,0,'@tile');wallRoom(0,6,24,28,8,P.white);
...
function practiceGuide(){
  ...
  if(s===2&&!held)body='方块放下了也没关系。先重新对准三点方块按 E 拿起，再平视按 + 推远放大。';
  if(s===3&&o&&o.userData.size<1.4)body='刚才又缩小了。E 拿起、抬到平视，按 + 放大到至少 1.40 m，再放到右侧圆环上。';
  if(s===4&&!held&&!aimTarget())body='走近三点方块，把准星放在它的表面，再按 P。这里只拍可操作物体，不拍整间房。';
  if(s===6&&!photoMode)body='照片收起来了。按 C 再举起，转向左侧空地，等预览显示“可显影”后按 F。';
  if(s===10&&!placed.some(p=>p.type==='bridge'))body='桥被收回了。Tab 切到桥照片，回到岸边白色脚印处，再用 C → F 把桥放回来。';
  if(s===9&&cards[selected]?.type!=='bridge')body='先按 Tab 切换到右下角的“桥”照片，再去白色脚印处，朝对岸用 C → F 放桥。';
...
function mechanicPrinciple(){
  ...
  return [
    ...
    '先用留白剪开幕布，再走进去。坐上观测椅才能对齐月亮；获得月光后，用回声与照片同时接通两处机关。'
  ][level]||'';
}
...
chapters.push({title:'先借一个不可能',sub:'00 / TRY THE RULES WITH YOUR OWN HANDS',goal:'一间可以随时重来的展厅。每次只试一个动作。',intro:'不用了解任何原作。先在这里亲手验证两条规则。',hints:['先移动到起步圈，再对准三点方块按 E。拿住后抬到平视，+ 或滚轮向上推远，实际尺寸会增加。','放大到至少 1.40 m，把方块放在右侧右侧实线圆环。接近圆环，低头调落点，E 松手，亮起即成功。','空手 P 拍方块，C 举照片，转向左侧空地，显示“可显影”时 F。随后按 X 收回副本，原件和照片保留。','领取桥照片后，站在断崖前起步圈，朝对岸用 C / F。走过真的桥，再进入出口。Esc 菜单可以直接前往第二章。']});
