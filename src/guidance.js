...
function chapterGuide(){
  ...
  if(level===0){
    if(!cards.some(c=>c.type==='stairs')){
      ...
      return step('reception-step','借一个够得着高台的台阶','对黄色方块按 E 拿起。平视、按 + 推远放大至约 1.2 m，再低头放在左侧高台前。空格跳上方块，再跳上高台。',guidePoint(-5,-3.1,.8),'台阶落点',1,4);
    }
    if(!placed.some(p=>p.type==='stairs'))return step('reception-stairs','让照片连接两层地面','站到大厅尽头的 DEVELOP 标记前，面朝出口。C 举照片；滚轮调距离，预览变绿后 F 显影。',guidePoint(0,-8,.2),'站在这里朝前显影',3,4);
    return guideExit(4,4);
  }
  if(level===1){
    if(flags.seenDoor)return guideExit(3,3);
    if(!pads[0].on)return step('back-anchor','在重复中留下一件小东西','E 拿黄色方块，按 − 拉近缩到 0.65 m 以下；放在左侧锚点中心，再松手。圆环亮起才算接通。',ps(0),'小尺寸锚点',1,3);
  ...
  if(level===2){
    ...
    if(!placed.some(p=>p.type==='stairs'))return step('pool-stairs','把楼梯接到中央高台','站在近岸标记前朝中央高台。C 举照片，滚轮调落点，绿色时 F。',guidePoint(0,4,.2),'显影站位',2,5);
    if(!align.done)return step('pool-align','登上高台，让三段弧线成为圆','走上楼梯，站在红色观察圈中心。移动视角对准远处圆弧，保持约 1.2 秒。',player.pos.distanceTo(align.pos)<1.3?align.target:align.pos,'观察位置 / 视线',3,5);
  ...
  if(level===3){
    ...
    if(cubes.length<2&&!guideCopy())return step('archive-capture','给唯一的方块留一份副本','把手里的东西放下，准星对准黄色方块，按 P。照片记住此刻的尺寸。',obj('cube'),'拍摄原件',2,5);
    if(cubes.length<2)return step('archive-copy','将照片变成第二个方块','C 举起照片，朝旁边的空地放置。绿色表示能放下，按 F 得到独立副本。',guidePoint(-3,4,.2),'空地',3,5);
  ...
  if(level===4){
    ...
    if(player.pos.x<25)return step('garden-door','带着光穿过门','对发光灯箱按 E 拿起，然后直接走过青色折叠门。',guidePoint(0,-8,1.6),'折叠门',1,4);
  ...
  if(level===5){
    ...
    if(!pads[0].on){if(!guideCopy('cube'))return step('final-copy','先保留黄色方块的副本','准星对准黄色方块，P 拍照。后面要让原件留在近岸，副本去对岸。',obj('cube'),'原件',1,7);return step('final-anchor','让原件守住近岸','E 拿方块，缩小到 ≤ 0.7 m，放在左侧锚点。这个圈必须一直亮着。',ps(0),'地址锚点',2,7)}
    if(!placed.some(p=>p.type==='bridge'))return step('final-bridge','接通前往对岸的桥','Tab 选桥照片，站在近岸标记前朝出口，C 举起、F 显影。',guidePoint(0,4,.2),'显影桥',3,7);
    if(!pads[1].on)return step('final-weight','把另一份重量送过去','Tab 选黄色方块照片，C / F 得到副本。E 带过桥，放大到 ≥ 1.8 m 后放入右侧圆环。',ps(1),'对岸重量',4,7);
  ...
  if(level===6){
    ...
    if(player.pos.x>25){if(!guideCopy('lamp'))return step('house-return','回屋外，补拍一份光','先通过身后的门返回：准星对准门，按 E。屋外对灯箱按 P，再进入房子。',guidePoint(38,8,1.7),'回到屋外',1,5);return step('house-light','把照片里的光带进庭院','Tab 选灯箱照片。靠近右侧接收器，C 举起，绿色时 F 显影。照片里的灯会成为真实光源。',sensors[0].pos,'光接收器',4,5)}
  ...
  if(level===8){
    ...
    if(!actorPads[2].on||!actorPads[3].on)return step('choir-effigy','先把人偶放进正确的位置','从后方“IV 留空”圈拿走红色人偶，缩到 ≤ 1.05 m，放进右边“III 人偶”圈。IV 圈要保持空着。',ap(2),'III · 人偶',1,4);
  ...
  if(level===9){
    ...
    if(!photo||photo.length<11){if(!flags.shadowAligned||shadowLength<12)return step('shadow-size','让人偶的影子够到对岸','E 搬动红色人偶，放在断崖前圆环上。放大至约 2.8 m，放下后影长应达到约 14 m。',guidePoint(0,2.5,.2),'人偶落点',1,5);return step('shadow-capture','拍摄地上的影子','先放下人偶，侧移到一旁再低头。让准星落在地面的长影上，按 P；照片应显示“影子道路”。',guidePoint(0,-2,.035),'拍这里的长影',2,5)}
    ...
    return step('shadow-develop','让影子的照片成为路','Tab 选影子，站到断崖前面向对岸。C 举起，滚轮调落点，绿色时 F 显影；再走上黑色道路。',guidePoint(0,4,.2),'朝对岸显影',4,5);
  }
  if(level===10){
    if(flags.moonExit)return guideExit(7,7);
    if(player.pos.x<25)return step('moon-screen','剪开公路幕布，走进画面','C 举留白负片，对准前方幕布的白弧标记；绿色时 F 剪开，再用 WASD 穿过入口。',guidePoint(0,-6.8,2.3),'幕布入口',1,7);
    if(!flags.moonRemembered){if(seated)return step('moon-align','坐稳，让圆弧围住月亮','移动鼠标环顾，让三段弧线重合。对准后保持约 1.2 秒；进度条满时月光会自动进入相册。',align.target,'月亮与圆弧',3,7);return step('moon-seat','沿弯路上行，在中央红椅坐下','沿 S 形道路走到高处庭院。靠近正中红椅，把准星对准椅子，按 E。站着观察不会得到月光。',guidePoint(42,-23,3.2),'中央红椅',2,7)}
    ...
    return step('moon-light','把月光显影在右边','Tab 切到“一小片月亮”，靠近右侧接收器。C 举照片，绿色时 F。让月光和回声同时停留约一秒。',sensors[0].pos,'月光接收器',6,7);
  }
...
function guideControls(){
  ...
  if(photoMode){const c=cards[selected];if(c?.type==='cutout')return [ghostValid?'F 剪开通道':'准星 → 白弧标记',ghostValid?'绿色：位置有效。按 F 剪开，C 放下照片。':'留白只能用于带白弧的墙；靠近并把准星放在标记上。'];return [ghostValid?'F 显影 · C 放下照片':'滚轮调距离 · Q / E 旋转',ghostValid?'绿色：可以成为实体。按 F 放置，X 可收回。':photoFailureReason()];}
